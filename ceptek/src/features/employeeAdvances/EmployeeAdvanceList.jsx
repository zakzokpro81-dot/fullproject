...
import {
  useEmployeeAdvanceQuery,
  useEmployeeAdvanceMutations,
  useEmployeeAdvanceFormOptions,
} from "./employeeAdvance.hooks";
import { employeeAdvanceColumns } from "./employeeAdvance.columns";
...
export function EmployeeAdvanceList() {
  ...
  const { employees, accounts } = useEmployeeAdvanceFormOptions();
  ...
  const handleFormSubmit = (data) => {
    const clean = { ...data };
    if (!clean.account_id) delete clean.account_id;
    mode === "add"
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <ConfirmDeleteDialog
        open={openDelete}
        itemName={`Advance #${selectedItem?.id || ""}`}
        onClose={() => {
          setOpenDelete(false);
          setSelectedItem(null);
        }}
