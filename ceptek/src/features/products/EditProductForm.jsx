...
import { Dialog, DialogTitle, DialogContent, DialogActions, Button, TextField, Autocomplete, CircularProgress, Box, Divider } from "@mui/material";
import { useForm, Controller } from "react-hook-form";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { getAttributes, getProductAttributes, updateProduct, getWarehouses, getProductStockLocation, getCategories } from "./product.api";
...
const EditProductForm = ({ open, onClose, product }) => {
    ...
    // Queries
    const { data: categories } = useQuery({ queryKey: ["categories"], queryFn: getCategories, enabled: open });
    const { data: warehouses } = useQuery({ queryKey: ["warehouses"], queryFn: getWarehouses, enabled: open });
   ...
   useEffect(() => {
    if (open && product && warehouses && attributes && currentAttrValues && categories) {
        ...
        if (product.category_id) {
            const currentCat = categories.find(c => Number(c.id) === Number(product.category_id));
        ...
        attributes.forEach(attr => {
            ...
            if (match) {
                if (attr.has_options && attr.options) {
                    const selectedOption = attr.options.find(opt => opt.value === match.value);
                    attrMap[attr.slug] = selectedOption || { value: match.value, label: match.value };
                } else {
        ...
        if (currentStockEntry?.warehouse) {
            ...
            const currentWarehouse = warehouses.find(w => Number(w.id) === Number(product.warehouse_id));
...
}, [open, attributes, currentAttrValues, warehouses, product, categories, setValue, currentStockEntry]);
            ...
            <DialogContent dividers>
                {!isReady ? (
                    ...
                    <>
                            ...
                            <Controller
                                ...
                                render={({ field }) => attr.has_options ? (
                                    <Autocomplete
                                        ...
                                        renderInput={(params) => <TextField {...params} label={attr.name} margin="normal" />}
                                    />
                                ) : (
                                    <TextField {...field} label={attr.name} margin="normal" fullWidth />
                                )}
                            />
                        ))}
                        ...
                        <Controller name="description" control={control} render={({ field }) => <TextField {...field} label="Description" multiline rows={2} margin="normal" fullWidth />} />
                        ...
                        <TextField
                            ...
                            value={currentStockEntry?.warehouse?.name || "No Warehouse Assigned"}
                            disabled
