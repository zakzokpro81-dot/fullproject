...
import {
  ...
  useJobTitleFormOptions,
} from "./jobTitle.hooks";
import { jobTitleColumns } from "./jobTitle.columns";
...
export function JobTitleList() {
  ...
  } = useJobTitleMutations({ onSuccess: handleCloseForm, showMessageDialog });
  const { departments } = useJobTitleFormOptions();
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box
        ...
        mb={2}
      >
        <Typography variant="h5">{t("jobTitles.title")}</Typography>
        <Box display="flex" gap={1}>
          ...
          <Button variant="contained" onClick={handleOpenAdd}>
            {t("common.addNew")} {t("jobTitles.entity")}
          </Button>
        </Box>
      </Box>
      <Box mb={2}>
