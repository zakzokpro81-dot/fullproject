...
// داخل مكون InvoiceDetailsDrawer
export default function InvoiceDetailsDrawer({ invoice, onClose }) {
    ...
    return (
        <Drawer
            ...
            open={!!invoice}
            onClose={onClose}
            PaperProps={{ sx: { width: { xs: "100%", sm: 500 } } }} // عرض أكبر قليلاً
        >
            {/* تقليل الـ Padding من 10 إلى 3 ليكون المحتوى مرئياً */}
            <Box sx={{ p: 10     }}>
                {/* Header */}
                ...
                <Paper variant="outlined" sx={{ p: 2, bgcolor: "#fafafa", mb: 3 }}>
                    <Typography fontWeight="bold" gutterBottom>Payment Summary</Typography>
                    <Stack spacing={1}>
                        <Box display="flex" justifyContent="space-between">
                            <Typography variant="body2">Total:</Typography>
                            <Typography variant="body2" fontWeight="bold">${invoice.total_amount}</Typography>
                        </Box>
                        <Box display="flex" justifyContent="space-between">
                            <Typography variant="body2">Paid:</Typography>
                            <Typography variant="body2" color="success.main">${invoice.paid_amount || 0}</Typography>
                        </Box>
                        <Divider />
                        <Box display="flex" justifyContent="space-between">
                ...
                <Typography fontWeight="bold" sx={{ mb: 1 }}>Items</Typography>
                ...
                {isLoading ? (
                    ...
                    <TableContainer component={Paper} variant="outlined">
                        <Table size="small">
                            ...
                            <TableBody>
                                {/* نستخدم items التي تأتي من useQuery */}
                                {items?.map((item, index) => (
                                    <TableRow
                                        ...
                                        sx={{ cursor: "pointer" }}
                                        onClick={() => {
                                            // الوصول للمعرف بنفس طريقة ملف الـ Orders
                                    ...
                                    >
                                        ...
                                        <TableCell align="center">
                                            {/* أضفنا السعر ليتناسب مع طبيعة الفواتير */}
