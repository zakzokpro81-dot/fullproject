...
import {
  usePayrollQuery,
  usePayrollMutations,
  usePayrollFormOptions,
} from "./payroll.hooks";
import { payrollColumns } from "./payroll.columns";
...
export function PayrollList() {
  ...
  } = usePayrollMutations({ onSuccess: handleCloseForm, showMessageDialog });
  const { employees } = usePayrollFormOptions();
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <ConfirmDeleteDialog
        open={openDelete}
        itemName={`Payroll #${selectedItem?.id || ""}`}
        onClose={() => {
          setOpenDelete(false);
          setSelectedItem(null);
        }}
